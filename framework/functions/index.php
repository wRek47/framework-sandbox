...
function structure($type, $nbt = false, $className = null) {

    $structures = cursor("common/database/cms/structures.json");

    $blank_structure = (object) array("type" => "unknown", "className" => null, "nbt" => false);
    $blank_nbt = (object) array("id" => "", "class" => "", "inner" => "");

    $inner_mesh = (object) array("type" => $type, "className" => $className, "nbt" => $nbt);
    $inner_nbt = clone $blank_nbt;

    if ($nbt):
    
        $inner_nbt->id = $nbt->id;
        $inner_nbt->class = $nbt->class;
        $inner_nbt->inner = $nbt->inner;
    
    endif;

    $structure = array_query($structures, $type, "type");
    if ($structure):
    
        if (!$structure->nbt): $structure->nbt = clone $blank_nbt; endif;
    
    endif;

    if (!$structure): $structure = clone $inner_mesh; endif;
    if (!$structure->nbt): $structure->nbt = $inner_nbt; endif;

    $is_class = false;
    if ($structure->className): $is_class = true;
    elseif ($className): $structure->className = $className; $is_maybe_class = true;
    endif;

    if ($is_maybe_class): // can we prove it should be or not?
    endif;

    if ($is_class): $resultInner = new $structure->className($structure->nbt->id, $structure->nbt->class, $structure->nbt->inner);
    else: $resultInner = $structure->nbt->inner;
    endif;

    $resultOuter = $structure;

    return [$resultOuter, $resultInner];
