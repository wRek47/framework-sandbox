...
if (!isset($_FW)):
	...
	define("FW", "framework/");
	define("FW_INDEX", FW . INDEX);
	
	require_once FW . "functions.php";
	require_once FW . "classes.php";
...
elseif (empty($_SPACES)):
	...
	table_system("visits", cursor("database/cms/traffic/visits.json"), $_TABLE['visits']);
	table_system("visits", cursor("database/cms/traffic/visitors.json"), $_TABLE['visitors']);
	...
	table_system("", cursor("database/cms/users.json"), $_TABLE['users']);
	...
	if ($fw->user['token']):
		...
		table_system("userProfile", cursor("database/cms/users/{$fw->user['token']->id}/profile.json"), $_TABLE['user_profile']);
