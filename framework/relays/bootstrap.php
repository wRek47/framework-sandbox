...
if (!defined("BOOTSTRAP")):
    ...
    define("BOOTSTRAP", "framework/relays/bootstrap.php");
...
if (isset($interface)):
    ...
    if (is_array($interface)):
        ...
        foreach ($currentInterface as $interface): include BOOTSTRAP; endforeach;
    ...
    elseif (is_object($interface)):
        ...
        elseif ($inner instanceOf BootstrapFormGroup): $formGroup = $inner; $inner = true;
        elseif ($inner instanceOf BootstrapFormGroupLabel): $formGroupLabel = $inner; $inner = true;
        elseif ($inner instanceOf BootstrapFormGroupField): $formGroupField = $inner; $inner = true;
...
elseif (isset($grid)):
    ...
    $interface = $currentGrid->inner; include BOOTSTRAP;
...
elseif (isset($coinSurface)):
    ...
    $interface = $currentCoinSurface->inner; include BOOTSTRAP;
    printf('</aside>' . PHP_EOL);
...
elseif (isset($container)):
    ...
    $interface = $currentContainer->inner; include BOOTSTRAP;

    printf('</div>' . PHP_EOL);

elseif (isset($card)):

    if (isset($currentCard)): $previousCard = $currentCard; unset($currentCard); endif;
    $currentCard = $card; unset($card);
    
    printf('<section class="%s">' . PHP_EOL, $currentCard->class);

    if ($currentCard->header): $interface = $currentCard->header; include BOOTSTRAP; endif;
    if ($currentCard->body): $interface = $currentCard->body; include BOOTSTRAP; endif;
    if ($currentCard->footer): $interface = $currentCard->footer; include BOOTSTRAP; endif;
        ...
        foreach ($currentCardBody->inner as $inner):
            ...
            if (is_string($inner)):
                ...
                if (is_file(ltrim($inner, "/"))):
                    ...
                    if ($pathinfo == "php"): unset($inner, $passed); include INDEX;
                    elseif ($pathinfo == "md"):
            ...
            if (isset($passed) AND !$passed): $interface = $inner; include BOOTSTRAP; endif;
...
elseif (isset($cardFooter)):
    ...
    $interface = $currentCardFooter->inner; include BOOTSTRAP;
